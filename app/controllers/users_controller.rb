class UsersController < ApplicationController
  ...
  def show
    ...
    @deletef = Friendship.where(friend_id: @user.id, user_id: current_user.id).or(Friendship.where(friend_id: current_user.id, user_id: @user.id)).where(confirmed: true).first
  end
end
